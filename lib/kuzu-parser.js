  ...
  if (data._id && data._label && !data._src) {
    ...
    if (!nodes.has(nodeId)) {
      ...
      nodes.set(nodeId, {
        ...
        style: { fill: getColorForLabel(data._label), stroke: '#11181C', lineWidth: 3, shadowBlur: 12, shadowOffsetX: 2, shadowOffsetY: 4, },
        kuzuData: {
          kuzu_id: data._id,
  ...
  // Edges
    ...
    if (!edges.has(edgeId)) {
      ...
      edges.set(edgeId, {
        ...
        target: targetId,
        label: data._label,
        style: { endArrow: true, stroke: '#5f5f5f', labelText: data._label, labelFill: '#5f5f5f', labelPlacement: 'center', labelTextBaseline: "bottom", },
        kuzuData: {
            kuzu_id: data._id,
            label_type: data._label,
            properties: properties,
