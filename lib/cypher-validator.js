...
/**
 * Valide le code Cypher pour la création de relations.
 * @param {string} cypher - Le code Cypher à valider.
 * @returns {boolean} - True si valide, sinon False.
 */
export function validateRelationshipCypher(cypher) {
  if (!cypher || cypher.trim() === '') return true; // Vide est valide

  const queries = cypher.split(/;\s*\n?/).map(q => q.trim()).filter(q => q.length > 0);
  for (const query of queries) {
    // Doit contenir le pattern MATCH... MERGE (a)-[r]->(b)
    const matchCount = (query.match(/\bMATCH\b/gi) || []).length;
    const mergeCount = (query.match(/\bMERGE\b/gi) || []).length;
    const hasRelPattern = /-\s*\[\s*:\s*\w+\s*\]\s*->/i.test(query);

    if (matchCount < 1 || mergeCount !== 1 || !hasRelPattern) {
        console.warn("Validation Relationship Failed: Must follow MATCH..., MERGE ()-[]->() pattern.", query);
        return false;
    }
    // Ne doit pas créer de nœuds directement
    if (/\bCREATE\s*\(/i.test(query)) {
        console.warn("Validation Relationship Failed: Should not use CREATE to make nodes.", query);
        return false;
    }
  }
  return true;
